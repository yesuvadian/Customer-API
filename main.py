...
# Corrective Action Request overdue check -- same "notify once per breach"
# design as _check_review_sla_breaches above (overdue_notified_at mirrors
# sla_breach_notified_at exactly, see migration 049's own comment). due_date
# is stamped on the CAR at creation from CAR_DUE_DAYS_CRITICAL/ALERT, or a
# per-rule CarTriggerConfig.car_due_in_days override (services/car_service.py)
# -- this job only reads it, never computes a deadline itself. Reassigning a
# CAR to a new due_date clears overdue_notified_at (routers/car.py), so a
# later deadline that also passes gets its own fresh notification.
def _check_car_overdue():
    db = BackgroundSessionLocal()
    try:
        from models import CorrectiveActionRequest, CarStatus
        from services.notification_service import NotificationService
        ...
        candidates = (
            ...
            .filter(
                CorrectiveActionRequest.status.in_(list(CarStatus.OPEN_STATUSES)),
                CorrectiveActionRequest.due_date.isnot(None),
                CorrectiveActionRequest.due_date < today,
                CorrectiveActionRequest.overdue_notified_at.is_(None),
            )
            .all()
        )
        ...
        for car in candidates:
            days_overdue = (today - car.due_date).days
            ...
            try:
                ...
                logger.warning(f"[Notif] CAR overdue fire failed for car={car.car_number}: {_e}")

            car.overdue_notified_at = now.replace(tzinfo=None)

        db.commit()
        if notified:
            logger.info(f"[Notif] CAR overdue check: {notified} notification(s) sent")
    except Exception as e:
        logger.error(f"[Notif] CAR overdue job error: {e}", exc_info=True)
    finally:
        db.close()
