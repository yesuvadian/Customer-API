...
class CategoryMasterService:
    ...
    def create_master_category(
        cls, db: Session, name: str, description: str | None = None, created_by: UUID | None = None
    ):
    ...
    def update_master_category(cls, db: Session, category_id: int, updates: dict):
        ...
        if new_name and new_name != category.name:
            existing = db.query(CategoryMaster).filter(
                CategoryMaster.name == new_name,
                CategoryMaster.id != category_id  # <--- exclude current category
            ).first()
            if existing:
                raise HTTPException(status_code=400, detail="Category Master with this name already exists")
