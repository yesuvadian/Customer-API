"""
Seed: CAR Trigger Config module + role privileges
====================================================================
...
Separate module from "corrective-action-requests" (the CAR dashboard) —
same relationship as "CM Recommendation Config" is separate from the CM
dashboard even though the two are related. This module is the admin CRUD
screen for car_trigger_configs / car_trigger_followups: which
equipment_type + test_type + severity combinations trigger a CAR, and what
follow-up actions (test/maintenance/inspection/repair) get auto-created.
"""
import uuid
...
MODULE_PATH = "car-trigger-config"
MODULE_NAME = "CAR Trigger Config"
MODULE_DESCRIPTION = (
    "Admin rules for which equipment type / test type / severity "
    "combinations auto-create a Corrective Action Request, and which "
    "follow-up test, maintenance, inspection, or repair actions get "
    "raised automatically when they do."
)
...
def seed_car_trigger_config_module():
    ...
    try:
        ...
        db.commit()
        print(f"\n[DONE] CAR Trigger Config module ready. Granted to {granted} new role(s).")
...
if __name__ == "__main__":
    seed_car_trigger_config_module()
