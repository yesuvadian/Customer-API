"""
Register the "CM Recommendation Config" module and grant full permissions to
every org-admin role.

Safe to run multiple times — skips insert if the module already exists.

Usage:
    python seed_recommendation_config_module.py
"""
import uuid
...
MODULE_PATH = "recommendation_config"
MODULE_NAME = "CM Recommendation Config"
    ...
    try:
        ...
        if existing:
            ...
            mod = Module(
                name=MODULE_NAME,
                description=(
                    "Manage score-band recommendation rules (equipment type, health "
                    "score range, test/maintenance type, frequency) that drive the "
                    "auto-generated schedule recommendations on the AI Analytic Dashboard."
                ),
                path=MODULE_PATH,
                group_name="Testing",
                is_active=True,
        ...
        db.commit()
        print(f"\n[DONE] CM Recommendation Config module ready. Granted to {granted} role(s).")
