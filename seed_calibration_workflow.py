"""
Seed calibration workflow stages, stage templates, transitions, and role mappings.

Source files (all at repo root):
  CALIBRATION_STAGE_TEMPLATES.json      — {template_key: {name, template_type, sections, ...}}
  CALIBRATION_STAGE_TEMPLATE_MAP.json   — {stage_name: template_key}
  CALIBRATION_STAGE_ROLES.json          — [{stage_code, roles, assign_also, assignment_role}]

Call order:
  1. seed_calibration_stages(db)                   — org-agnostic; run once during full seed
  2. seed_calibration_role_mappings(db, org_id)    — org-specific; run per organization

Lifecycle:
  CAL_REVIEW      → (approve) → CAL_EXECUTION
  CAL_EXECUTION   → (approve) → CAL_CERTIFICATE
  CAL_CERTIFICATE → (approve) → CAL_VERIFY
  CAL_VERIFY      → (approve) → [closed / terminal]
  CAL_VERIFY      → (reject)  → CAL_CERTIFICATE
"""
...
STAGES = [
    {"code": "CAL_REVIEW",      "name": "Calibration Due Review", "sequence": 1, "default_duration_days": 3},
    {"code": "CAL_EXECUTION",   "name": "Send to Lab",            "sequence": 2, "default_duration_days": 14},
    {"code": "CAL_CERTIFICATE", "name": "Certificate Upload",     "sequence": 3, "default_duration_days": 5},
    {"code": "CAL_VERIFY",      "name": "Officer Verification",   "sequence": 4, "default_duration_days": 3},
]
...
TRANSITIONS = [
    ...
    ("CAL_EXECUTION",   "approve", "CAL_CERTIFICATE"),
    ("CAL_CERTIFICATE", "approve", "CAL_VERIFY"),
    ("CAL_VERIFY",      "approve", None),               # terminal — calibration closed
    ("CAL_VERIFY",      "reject",  "CAL_CERTIFICATE"),  # Officer rejects → re-upload cert
]
...
def seed_calibration_stages(db) -> int:
    """
    Idempotently insert:
      - RepairWorkflowDefinition  (CALIBRATION)
    ...
    """
    templates_raw  = _load("CALIBRATION_STAGE_TEMPLATES.json")
    stage_tmpl_map = _load("CALIBRATION_STAGE_TEMPLATE_MAP.json")
    ...
    if not wf_def:
        ...
        db.add(wf_def)
        db.flush()
    print(f"[OK] RepairWorkflowDefinition CALIBRATION: {wf_def.id}")
    ...
    db.flush()
    print(f"[OK] Calibration stage templates: {len(template_map)} ready")
    ...
    print(f"[OK] Calibration stages: {inserted} inserted ({len(stage_map)} total)")
    ...
    db.commit()
    print("[OK] Calibration stages seeded successfully")
    return inserted


def seed_calibration_role_mappings(db, organization_id) -> int:
    """
    Idempotently insert/update RepairStageRole rows for the given organization.

    Must be called after seed_calibration_stages so stage rows exist.
    Returns the number of role mapping rows upserted.
    """
    roles_raw = _load("CALIBRATION_STAGE_ROLES.json")
    upserted  = 0
    ...
    for entry in roles_raw:
        ...
        if not stage:
            print(f"[WARN] Stage not found: {stage_code} — run seed_calibration_stages first")
            continue
    ...
    db.commit()
    print(f"[OK] Calibration role mappings: {upserted} upserted for org {organization_id}")
    return upserted
