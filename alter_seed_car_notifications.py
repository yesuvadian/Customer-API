#!/usr/bin/env python3
"""
One-time setup: register the Corrective Action Request notification events
-- car_created, car_assigned -- so they're fully leverage-able from the
existing Notification Center screen (Organisation -> Notification Center),
same as every other event in the system. Adds:
...
(car_overdue was a third event here originally but was removed -- it
duplicated the existing generic "Test Overdue" alert, which already fires
for a CAR's auto-created follow-up TestingRequest once it passes its own
due_date. See migration 051.)
...
def main():
    ...
    try:
        ...
        for event_type in ("car_created", "car_assigned"):
