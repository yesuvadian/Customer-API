...
def upload_grn(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Unexpected error uploading GRN: {str(e)}")
...
def update_grn(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating GRN: {str(e)}")
...
def get_grn_data(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
        # Use the lightweight cached path — no customer_id scoping needed here
...
def delete_grn(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
        # Fetch once — reuse for both delete_attachment_by_prefix and grn field clear
        salesorder = sales_order_service._get_order_status(access_token, salesorder_id)

        sales_order_service.delete_attachment_by_prefix(
            access_token=access_token,
            salesorder_id=salesorder_id,
            prefix="_grn",
            salesorder=salesorder,  # avoids second GET
        )
        # Clear GRN number field (cache is invalidated inside update_grn_number_field)
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting GRN: {str(e)}")
...
def get_grn_pdf(salesorder_id: str, current_user=Depends(get_current_user)):
    ...
    pdf_bytes = sales_order_service.get_attachment_pdf_by_prefix(
        access_token, salesorder_id, prefix="_grn"
    )
...
def upload_po(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Unexpected error uploading PO: {str(e)}")
...
def update_po(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error updating PO: {str(e)}")
...
def delete_po(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
        # Fetch once — pass to both operations to avoid two GETs
        salesorder = sales_order_service._get_order_status(access_token, salesorder_id)
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting PO: {str(e)}")
...
def delete_po_number(salesorder_id: str, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error deleting PO number: {str(e)}")
...
def get_po_pdf(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    pdf_bytes = sales_order_service.get_attachment_pdf_by_prefix(
        access_token, salesorder_id, prefix="_po"
    )
    return Response(content=pdf_bytes, media_type="application/pdf")
...
def list_attachments(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
        # Use lightweight cached path — documents are present in the SO response
        salesorder = sales_order_service._get_order_status(access_token, salesorder_id)
    except Exception as e:
    ...
    attachments = [
        ...
        for doc in salesorder.get("documents", [])
    ]
...
def download_attachment(
    ...
    access_token = get_zoho_access_token()
    file_bytes = sales_order_service.download_attachment(access_token, salesorder_id, attachment_id)
...
def delete_attachment(
    ...
    access_token = get_zoho_access_token()
    return sales_order_service.delete_attachment(access_token, salesorder_id, attachment_id)
...
def get_comments(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
        comments = sales_order_service.get_comments(access_token=access_token, salesorder_id=salesorder_id)
    except Exception as e:
...
def delete_comment(
    salesorder_id: str, comment_id: str, current_user=Depends(get_current_user)
):
    access_token = get_zoho_access_token()
    try:
...
def review_order(
    salesorder_id: str,
    payload: zohoschemas.ReviewSalesOrder,
    current_user=Depends(get_current_user),
):
    access_token = get_zoho_access_token()
    try:
...
def approve_order(
    salesorder_id: str,
    payload: zohoschemas.ApproveSalesOrder,
    current_user=Depends(get_current_user),
):
    access_token = get_zoho_access_token()
    try:
...
def get_order_pdf(salesorder_id: str, current_user=Depends(get_current_user)):
    ...
    try:
        pdf_bytes = sales_order_service.get_order_pdf(
            access_token=access_token, salesorder_id=salesorder_id
        )
    except HTTPException as e:
        raise e
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching sales order PDF: {str(e)}")
    return Response(content=pdf_bytes, media_type="application/pdf")
...
def get_eway_bill(salesorder_id: str, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching E-Way Bill: {str(e)}")
    return Response(content=pdf_bytes, media_type="application/pdf")
...
def get_vendor_shipment_details(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
...
def create_po_from_salesorder(salesorder_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
...
def create_salesorder_from_quote(estimate_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    try:
        ...
        if not salesorder:
            raise HTTPException(status_code=400, detail="Sales order was not created")
        return {"message": "Sales order created successfully", "salesorder": salesorder}
    except HTTPException as http_error:
        raise http_error
    except Exception as e:
        raise HTTPException(status_code=500, detail="Internal server error while creating sales order")
