...
def review_invoice(
    invoice_id: str,
    payload: zohoschemas.ReviewInvoice,
...
def approve_invoice(
    invoice_id: str,
    payload: zohoschemas.ApproveInvoice,
    current_user=Depends(get_current_user),
):
    access_token = get_zoho_access_token()
    try:
...
def list_invoice_attachments(invoice_id: str, current_user=Depends(get_current_user)):
    access_token = get_zoho_access_token()
    # Reuses the cached get_invoice — no separate Zoho call needed
    attachments = invoice_service.get_invoice_attachments(
        access_token, invoice_id, contact_id=current_user.email
    )
...
def download_invoice_attachment(
    ...
    access_token = get_zoho_access_token()
    file_bytes = invoice_service.download_invoice_attachment(access_token, invoice_id, attachment_id)
...
def update_invoice_comment(
    ...
    try:
        updated = invoice_service.update_invoice_comment(
            access_token, invoice_id, comment_id, payload, email=current_user.email
        )
    except Exception as e:
...
def delete_invoice_comment(
    invoice_id: str, comment_id: str, current_user=Depends(get_current_user)
):
    access_token = get_zoho_access_token()
    try:
